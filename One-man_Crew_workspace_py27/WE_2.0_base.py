...
def construct_heu_feasi_sol(G, max_speed = None, dist_first = True, rand = False):   
    ...
    while set(G_c.nodes())-set(term_begin_list)-set(term_end_list)!= traversed_nodes and indicator < len(term_begin_list):
        ...
        indicator = 0 
        for ind in range(len(term_begin_list)):
            ind = random.choice(range(len(term_begin_list)))
    ...
    return_list = [solution, 
                   {'Terminals':len(solution.keys()),
                    ...
                    'Remained Nodes':len(G.nodes())-len(visited)
                    }
                   ]
    return return_list

def path_release(G, sol_dic, max_speed = None, release_condition = 1):
    print ('    ****Release Simple Conductor Trips to Others')
    G_c = G.copy()
...
def meta_heu(G, sol_dic, max_speed = None):
    print ('********Meta Heuristic Swapping to Reduce Average Speed')
    G_c = G.copy()
    path_list = []
    edge_list = []
    for k,v in sol_dic.iteritems():
    ...
    while avg_speed[-1] != avg_speed[-2]:
        ct += 1
        print 'number of meta-heuristic traverse run: ' + str(ct)
    ...
    avg_spd_first = avg_speed[2:-1][0]
    avg_spd_last = avg_speed[2:-1][-1]
    print ("initial average speed: "+str(avg_spd_first))
    print ("initial average speed: "+str(avg_spd_last))
    solution = transfer_graph_to_solution_dic(G_c)
    visited = set()
    t_c_number = {}
...
def plot_t_x_plot(G_origin, solu, dot_line=False, plt_ttl = True, width = 2000, height = 1500, save = False, direc = None, montecarlo_time = None):
    ...
    if plt_ttl:
        ...
        for k,v in attr_dic.iteritems():
            if k == 'Title':
                ttl = ttl + k +':  ' + str(v) +';'
            if k == 'Terminals':
                plt.text(terminal_loc[-1], shift_hour+2, k+':  '+str(v))
            if k == 'Conductor Number':
                plt.text(terminal_loc[-1], shift_hour+1, k+':  '+str(v))
            if k == 'Remained Nodes':
                plt.text(terminal_loc[-1], shift_hour+1.5, k+':  '+str(v))
            if k == 'Avg Speed':
                plt.text(terminal_loc[-1], shift_hour+2.5, k+':  '+str(v))
            if k == 'Estimated Cost':
                plt.text(terminal_loc[-1], shift_hour+0.5, k+':  '+str(v))
            
        plt.title(ttl)
    
            
    if save == True:
        fig.savefig(direc+'\\'+'figure'+'\\'+str(montecarlo_time)+'.png', dpi=200)
    else:
        fig.show()
    #-------plot edge speed-------------#
...
#################play with parameters here#################
        ...
        for t,p in solution[0].iteritems():
            conductor_number += len(p)
        avg_spd     = avg_speed(G, sol_dic=solution[0])
        ttl_dist    = avg_spd       * shift_hour        * conductor_number
        fuel_cost   = ttl_dist      / mpg               * fuel_price
        salary_cost = hourly_salary * conductor_number  * shift_hour
        ttl_cost_list.append(fuel_cost+salary_cost)
        
        spd_list.append(avg_spd)
        solution_list.append(solution)
        solution[1]['Avg Speed']        = str(round(avg_spd, 2)) + ' mph'
        solution[1]['Estimated Cost']   = '$'+str(round(fuel_cost+salary_cost, 0)) 
        
        print ('  Average speed for this Monte Carlo run is: '      + str(round(avg_spd,2))     + ' mph')
        print ('  Conductor number for this Monte Carlo run is: '   + str(conductor_number)) 
        print ('  Total cost for this Monte Carlo run is: $'        + str(round(fuel_cost+salary_cost,0)))
        
    sorted_solution = [x for _,x in sorted(zip(ttl_cost_list,solution_list))]
    spd_list        = sorted(spd_list)
    ttl_cost_list   = sorted(ttl_cost_list)
    
    meta_solution = meta_heu(G, sol_dic=sorted_solution[0][0], max_speed=max_speed)
    
    for t,p in meta_solution[0].iteritems():
        conductor_number += len(p)
    avg_spd     = avg_speed(G, sol_dic=meta_solution[0])
    ttl_dist    = avg_spd       * shift_hour        * conductor_number
    fuel_cost   = ttl_dist      / mpg               * fuel_price
    salary_cost = hourly_salary * conductor_number  * shift_hour
    ttl_cost_list.append(fuel_cost+salary_cost)
    
    meta_solution[1]['Avg Speed']       = str(round(avg_spd, 2)) + ' mph'
    meta_solution[1]['Estimated Cost']  = '$'+str(round(fuel_cost+salary_cost, 0)) 
    
    sorted_solution.insert(0, meta_solution)
    spd_list.insert(0, avg_spd)
    ttl_cost_list.insert(0,fuel_cost+salary_cost)
    print ('------Monte Carlo ('+str(n)+' Times) Completed------')
    return (sorted_solution, spd_list, ttl_cost_list)



monte_carlo_result  = monte_carlo()
solution            = monte_carlo_result[0][0]
fnl_speed           = monte_carlo_result[1][0]
fnl_cost            = monte_carlo_result[2][0]

for s in range(len(monte_carlo_result[0])):
    monte_carlo_result[0][s][1]['Title'] = 'Monte Carlo Simulation Results, N=100, i='+str(s)
    plot_t_x_plot(G, solu=monte_carlo_result[0][s], dot_line=False, plt_ttl = True, width = 1600, height=1200, save= True, direc=absolute_directory, montecarlo_time='Sorted_'+str(s))
...
for t in t_c_number:
    print '  Final number of conductors for Terminal_'+str(t[1])+' is: '+str(t_c_number[t])
    t_nmbr += t_c_number[t]


print ('  Final number of terminals: '+ str(len(t_c_number.keys())))
print ('  Final number of conductors: '+ str(t_nmbr))
print ('  Final number of remained nodes: '+str(len(G.nodes())-len(visited)))
print ('  Final conductor average speed: '+str(round(fnl_speed,2))+' mph')
print ('  Final estimated cost: $'+str(round(fnl_cost,0)))

endt = time.time()
print ('runtime: '+ str(endt-startt)+' Seconds')
print ('*************************program ends*************************')
